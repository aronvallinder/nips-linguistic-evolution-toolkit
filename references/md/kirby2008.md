...
*School of Philosophy, Psychology, and Language Sciences, University of Edinburgh, Edinburgh EH8 9LL, United Kingdom; and [‡] Division of Psychology,
Northumbria University, Newcastle-upon-Tyne NE1 8ST, United Kingdom


Edited by Dale Purves, Duke University Medical Center, Durham, NC, and approved June 6, 2008 (received for review August 20, 2007)



**We introduce an experimental paradigm for studying the cumu-**
...
# T Smith and Szathmary (1) as the most recent of a small numberhe emergence of human language has been cited by Maynard
...
language enables an entirely new system for information transmission: human culture. Language is unique in being a system that
...
Although this feature of language as a carrier of cultural information obviously is important, we have argued that there is a second
sense in which language is an evolutionary milestone: each utterance has a dual purpose, carrying semantic content but also
...
acquisition and processing of language are relatively well understood, and because language has an interesting, nontrivial, but well
...
**Iterated Learning.** Iterated learning is a process in which an individual acquires a behavior by observing a similar behavior in
...
cumulative and is not considered to arise from the explicit intentions of the individuals involved. Rather, this type of cultural
...
growing body of computational models of the emergence of multiword utterances from unstructured randomness (5, 8, 10, 11, 15),
...
claim that iterated learning leads to adaptively structured languages. It should demonstrate whether cumulative adaptive evolution without intention is possible purely by virtue of cultural
transmission.
In this paper, we implement such a paradigm and demonstrate
cumulative, adaptive, nonintentional cultural evolution of an artificial language in a laboratory population of human participants.


**Diffusion Chains.** Diffusion-chain studies provide the best example
of experimental treatments of iterated learning. In these experiments a participant observes some target behavior (provided by the
...
Author contributions: S.K., H.C., and K.S. designed research; H.C. performed research; S.K.,
H.C., and K.S. analyzed data; and S.K. wrote the paper.
...
[This article contains supporting information online at www.pnas.org/cgi/content/full/](http://www.pnas.org/cgi/content/full/0707835105/DCSupplemental)
[0707835105/DCSupplemental.](http://www.pnas.org/cgi/content/full/0707835105/DCSupplemental)
...
www.pnas.org�cgi�doi�10.1073�pnas.0707835105 PNAS � **August 5, 2008** � vol. 105 � no. 31 � **10681–10686**


behavior for a third participant, and so on. (We refer to each
...
this approach systematically (18–24) _._
The most recent, and arguably the most significant, instance of
a diffusion-chain experiment is the work of Horner _et al._, which
explores the cultural transmission of tool-use strategies in populations of chimpanzees and children (24). Diffusion chains are set up
in which an experimenter demonstrates 1 of 2 possible techniques
for opening a puzzle box (‘‘artificial fruit’’) to a participant. Subsequent participants observe their predecessor’s box-opening behavior and then in turn become the model for the next generation.
...
comparison with human language, we replicate the basic diffusionchain design with a more complex artificial-language learning task
...
This work bears some resemblance to a recent body of experimental work on the shared construction of communication systems
(27–30). Of particular relevance is a recent paper by Selten and
Warglien (30) that demonstrates that pairs of participants sometimes can create structured and efficient communication systems
...
experiments, as in those of Galantucci (27) and Garrod _et al._ (28,
...
of lowercase letters (see Fig. 1 for an example and the _Methods_
section for more details).
For training purposes, the language to be learned (a set of
string–picture pairs) is divided randomly into 2 sets of approximately equal size: the SEEN set and the UNSEEN set. A
participant is trained on the SEEN set, being presented repeatedly with each string–picture pair in random order (see _Methods_
for details). During subsequent testing, participants are presented with a picture and asked to produce the string they think
...
participants at adjacent generations (see _Methods_ ). The Levenshtein edit distance (31) between pairs of words (i.e., the smallest
number of character insertions, replacements, and deletions required to transform 1 word into the other) provides a reasonable
theory-neutral measure of distance. We normalized the edit distance for length of words so that identical strings have a distance of
...
this alignment between meaning and signal differs from the alignment we would expect to see by a random, unstructured assignment
of signals to meanings (see _Methods_ for details).





**10682** � www.pnas.org�cgi�doi�10.1073�pnas.0707835105 Kirby _et al._
...
**Fig. 2.** Transmission error and a measure of structure by generation in 4 chains. _a_ shows the increase in learnability (decrease in error) of languages over time. _b_ shows
structure in the languages increasing. The dotted line in _b_ gives the 95% confidence interval so that any result above this line demonstrates that there is a nonrandom
...
therefore is not plotted (see _Methods_ ). The language discussed in the paper is circled.



**Results of Experiment 1.** The results of our first experiment, involving 4 separate diffusion chains of 10 participants each, are shown in
...
decrease 0.748, SD � 0.147; t (3) � 8.656; _P_ � 0.002). This decrease
...
generalize to unseen meanings successfully, there must be systematic underspecification.
...
by generation 4 in 1 of the diffusion chains, the string _tuge_ is used
...
and unpredictable at this stage. By generation 6, _poi_ is used to refer
to most spiraling pictures, but there are exceptions for triangles and
squares. Blue spiraling triangles or squares are referred to as _tupin_,
and red spiraling triangles or squares are _tupim_ . In the following
...
horizontally is _tuge_, all spiraling objects are _poi_, and bouncing
...
correctly pictures that they have never previously seen. This generalization directly ensures the stable cultural transmission of the
...
Kirby _et al._ PNAS � **August 5, 2008** � vol. 105 � no. 31 � **10683**
...
become more structured. As can be seen in Fig. 2 _b_, significantly
...
generation are significantly more structured than the initial languages (mean increase 5.578, SD � 2.968, t (3) � 3.7575, _P_ � 0.02).
...
unambiguously. Such a language cannot rely on systematic underspecification of meanings but instead must find some other means
...
random) was removed from the training data. This filtering effectively removes the possibility of the language adapting to be
learnable by introducing underspecification: filtering ensures that
underspecification is an evolutionary dead-end. This process, although artificial, is an analogue of a pressure to be expressive that
...
the overall number of words in participants’ output remains comparatively high throughout the experiment, as shown in Table 2. Fig.
4 _a_ shows how transmission error changes as the language evolves.
Once again, it is clear that the languages are becoming more
learnable over time (mean decrease 0.427, SD � 0.106, t (3) �
8.0557, _P_ � 0.002) although it is not possible to introduce the kind
...
from Fig. 4 _b_ that, as in Experiment 1, the languages are becoming
increasingly structured over time (mean increase, 6.805, SD �
5.390, t (3) � 2.525, _P_ � 0.05). Because filtering rules out the
generalizations that emerged in the previous experiment, a different kind of structure that does not rely on underspecification must
be emerging.
...
and movement, respectively, with 1 exceptional irregularity ( _renana_
...
[lost or are reanalyzed from generation to generation [see support-](http://www.pnas.org/cgi/data/0707835105/DCSupplemental/Supplemental_PDF#nameddest=ST1)
[ing information (SI) Tables S1–S8 for the complete set of](http://www.pnas.org/cgi/data/0707835105/DCSupplemental/Supplemental_PDF#nameddest=ST1)
languages].


**Discussion**

What we have observed here under laboratory conditions is cumulative cultural adaptation without intentional design. Just as
...
**Fig. 4.** Transmission error and structure by generation in the experiment in which ambiguous data were removed from the training set at each generation. _a_ gives
error for the whole language; _b_ gives structure. These results show that, despite the blocking of underspecification, structure still evolves that enables the languages
to become increasingly learnable. The language discussed in the paper is circled.


**10684** � www.pnas.org�cgi�doi�10.1073�pnas.0707835105 Kirby _et al._


previous computational models have predicted (4–13), the culturally evolving language has adapted in a way that ensures its
successful transmission from generation to generation, despite the
existence of a bottleneck on transmission imposed by the incomplete exposure of each participant to the language. Cultural adaptation results in languages that circumvent this transmission problem by exploiting structure in the set of meanings to be conveyed.
...
it reflected the distinctions in meaning that the aliens were interested in communicating. The collapse of distinctions based on color
(which eventually occurred in all 4 replications of the first experiment) in favor of distinctions based on shape and movement is
...
Systematic underspecification similar to that found in the experiments is an important feature of natural language. For example, in
...
species-unique property of compositionality in syntax and morphology. [¶] The meaning of an expression normally is a function of the
...
requirement is enforced simply by filtering out ambiguous meaningstrings from the data given to the learner, leaving a training set with
...
[from generation to generation once they had emerged (see Tables](http://www.pnas.org/cgi/data/0707835105/DCSupplemental/Supplemental_PDF#nameddest=ST1)
[S1–S8 for the full set of languages). The difference between these](http://www.pnas.org/cgi/data/0707835105/DCSupplemental/Supplemental_PDF#nameddest=ST1)
...
faithfully from generation to generation, a language in this experiment must be both learnable and unambiguous. The learnability
...
evolution of this structure reveals a key feature of cultural transmission: it gives rise to adaptive systems that respond to the
pressures imposed by the transmission bottleneck that exists between the producer and learner of behavior. Crucially, this adaptation by the language maximizes its own transmissibility, and the
...
would be no reason we would find rampant structured underspecification in the first experiment and a system of morphological
...
language faces the problem of being reproducible from a subsample. In the first experiment, the language solves this problem by
introducing systematic underspecification in the meaning-signal
mapping. In the second experiment, the language faces the additional challenge of being transmitted despite filtering for ambiguity.
...
evolution can deliver the appearance of design without the existence of a designer, so too can cultural evolution.



Kirby _et al._ PNAS � **August 5, 2008** � vol. 105 � no. 31 � **10685**
...
(see _[SI Text](http://www.pnas.org/cgi/data/0707835105/DCSupplemental/Supplemental_PDF#nameddest=STXT)_ ). During training, participants were presented with string–picture
...
object with an arrow indicating motion. Each object feature (shape, color, motion) varied over 3 possible values: square, circle, or triangle; black, blue, or red;
...
1. Maynard Smith J, Szathmary E (1997) _The Major Transitions in Evolution_ (Oxford Univ
Press, Oxford, UK).
2. Deacon TW (1997) _The Symbolic Species: The Co-evolution of Language and the Brain_
(WW Norton, New York).
3. Christiansen MH, Chater N (2008) Language as shaped by the brain. _Behav Brain Sci_, in press.
4. Kirby S (1999) _Function, Selection and Innateness: The Emergence of Language_
_Universals_ (Oxford Univ Press, Oxford UK).
5. Hurford JR (2000) Social transmission favours linguistic generalization, _The Evolutionary_
_Emergence of Language: Social Function and the Origins of Linguistic Form_, eds Knight
C, Studdert-Kennedy M, Hurford JR (Cambridge Univ Press, Cambridge, UK), pp 324–352.
6. Kirby S, Smith K, Brighton H (2004) From UG to universals: Linguistic adaptation
through iterated learning. _Stud Lang_ 28:587–607.
7. Kirby S, Dowman M, Griffiths T (2007) Innateness and culture in the evolution of
language. _Proc Natl Acad Sci USA_ 104:5241–5245.
8. Kirby S (2000) Syntax without natural selection, _The Evolutionary Emergence of_
_Language: Social Function and the Origins of Linguistic Form_, eds Knight C, StuddertKennedy M, Hurford JR (Cambridge Univ Press, Cambridge, UK), pp 303–323.
9. Brighton H (2002) Compositional syntax from cultural transmission. _Artif Life_ 8:25–54.
10. Zuidema W (2003) How the poverty of the stimulus solves the poverty of the stimulus,
_Advances in Neural Information Processing Systems 15 (Proceedings of NIPS’02_ ), eds
Becker S, Thrun S, Obermayer K (MIT Press, Cambridge, MA), pp 51–58.
11. Smith K, Brighton H, Kirby S (2003) Complex systems in language evolution: The
cultural emergence of compositional structure. _Adv Complex Syst_ 6:537–558.
12. Brighton H, Smith K, Kirby S (2005) Language as an evolutionary system. _Physics of Life_
_Reviews_ 2:177–226.
13. Griffiths TL, Kalish ML (2007) Language evolution by iterated learning with Bayesian
agents. _Cogn Sci_ 31:441–480.
14. Keller R (1994) _On Language Change: The Invisible Hand in Language_ (Routledge,
London).
15. Batali J (2002) The negotiation and acquisition of recursive grammars as a result of
competition among exemplars, _Linguistic Evolution through Language Acquisition:_
_Formal and Computational Models_, ed Briscoe EJ (Cambridge Univ Press, Cambridge,
UK), pp 111–172.
16. Bickerton D (2007) Language evolution: a brief guide for linguists. _Lingua_ 117:510–526.
17. Bartlett FC (1932) _On Remembering_ (Macmillan, Oxford).
18. Curio E, Ulrich E, Vieth W (1978) Cultural transmission of enemy recognition: One
function of avian mobbing. _Science_ 202:899–901.



aresult,thetrainingdataseenbyparticipantsinthesecondexperimentconsisted
...
trainedontheoutputofthefinaltestingofthepreviousparticipant:theprevious
...
output gives a measure of intergeneration transmission error, and is given by


1
## E � i � � | M | m [�] � M LD� s im, s mi � 1 �


where _s_ _im_ isthestringassociatedwithmeaning _m_ bytheparticipantatgeneration
_i_, LD _s_ _im_, _s_ _jm_ is the normalized Levenshtein distance (31) between strings _s_ _im_ and _s_ _jm_,
and the sum is over a set of meanings _M_ of magnitude  _M_  .
...
differing in 2 features have a distance of 2, and so forth). The Pearson’s productmoment correlation between these 2 sets of distances then is calculated, giving
...
of the strings over meanings. The graphs shown in the paper give the _z_ score for
...
whenthelanguagehasonlythesamestringforallmeaningsorforallbut1ofthe
meanings. In these cases, all possible reorderings are equally structured.


**ACKNOWLEDGMENTS.** H.C. was supported by an Economic and Social Research
Council 1 � 3 Studentship. K.S. was supported by a British Academy postdoctoral
fellowship.


19. Laland KN, Plotkin HC (1990) Social learning and social transmission of foraging
information in Norway rats ( _Rattus norvegicus_ ). _Anim Learn Behav_ 18:246–251.
20. Galef BG, Jr., Allen C (1995) A new model system for studying animal traditions. _Anim_
_Behav_ 50:705–717.
21. Laland KN, Williams K (1997) Shoaling generates social learning of foraging information in guppies. _Anim Behav_ 53:1161–1169.
22. Mesoudi A, Whiten A (2004) The hierarchical transformation of event knowledge in
human cultural transmission. _J Cognit Cult_ 4:1–24.
23. Mesoudi A, Whiten A, Dunbar RIM (2006) A bias for social information in human
cultural transmission. _Br J Psychol_ 97:405–423.
24. Horner V, Whiten A, Flynn E, de Waal FBM (2006) Faithful replication of foraging
techniques along cultural transmission chains by chimpanzees and children. _Proc Natl_
_Acad Sci USA_ 103:13878–13883.
25. Gomez RL, Gerken L (2000) Infant artificial language learning and language acquisition. _Trends Cogn Sci_ 4:178–186.
26. Hudson-Kam C, Newport E (2005) Regularizing unpredictable variation: The roles of
adult and child learners in language formation and change. _Lang Learn Dev_ 1:151–195.
27. GalantucciB(2005)Anexperimentalstudyoftheemergenceofhumancommunication
systems. _Cogn Sci_ 29:737–767.
28. Garrod S, Doherty G (1994) Conversation, co-ordination and convention: An empirical
investigation of how groups establish linguistic conventions. _Cognition_ 53:181–215.
29. Garrod S, Fay N, Lee J, Oberlander J, MacLeod T (2007) Foundations of representation:
Where might graphical symbol systems come from? _Cogn Sci_ 31:961–987.
30. Selten R, Warglien M (2007) The emergence of simple languages in an experimental
coordination game. _Proc Natl Acad Sci USA_ 104:7361–7366.
31. Levenshtein VI (1966) Binary codes capable of correcting deletions, insertions and
reversals. _Sov Phys Dokl_ 6:707–710.
32. Landau B, Smith LB, Jones SS (1988) The importance of shape in early lexical learning.
_Cogn Dev_ 3:299–321.
33. Slobin DI (1977) in _Language Learning and Thought_, Macnamara J ed (Academic,
London), pp 185–221.
34. Smith K (2004) The evolution of vocabulary. _J Theor Biol_ 228:127–142.
35. Von Frisch K (1974) Decoding the language of the bee. _Science_ 185:663–668.
36. Zuberbu¨ hler K (2002) A syntactic rule in forest monkey communication. _Anim Behav_
63:293–299.



**10686** � www.pnas.org�cgi�doi�10.1073�pnas.0707835105 Kirby _et al._
